...
const Login = () => {
  ...
  return (
    <div className="relative flex flex-col h-screen w-screen bg-black md:items-center md:justify-center md:bg-transparent ">
      ...
      <Image src={"/assets/logo.png"} height={75} width={150} loading="lazy" alt="" className="absolute left-4 top-4 object-contain md:left-10 md:top-6" />
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="relative mt-24 space-y-8 rounded bg-black/75 py-10 px-6 md:mt-0 md:max-w-md md:px-14 ">
        ...
        <div className="space-y-4">
          ...
          <label className="inline-block w-full">
            ...
            {errors.password && <span className="p-1 text-[13px] font-light text-orange-500"> {errors && errors.password.message || "Wrong password"}</span>}
          </label>
        </div>
        ...
        <div className="flex text-[gray]">
          <p>New to Marflix?</p>
          <button type="submit" className="text-white ml-2 font-semibold hover:underline" onClick={() => setLogin(false)}>Sign up now</button>
        </div>
      </form>
    </div>
  )
